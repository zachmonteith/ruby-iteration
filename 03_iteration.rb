...
class Array
  ...
  def bubble_sort(&prc)
    return self.dup.bubble_sort! unless prc
    ...
    result.length.times do
      ...
      while i < result.length - 1
          if prc.call(result[i], result[i + 1]) == 1
  ...
  while start < string.length
    finish = start
      while finish < string.length
        result << string[start..finish] unless result.include?(string[start..finish])
        finish += 1
      end
...
def subwords(word, dictionary)
  dictionary.select { |checking_word| word.include?(checking_word)}
end
...
def concatenate(strings)
  strings.inject("") {|accumulator, name| accumulator + name }
end
